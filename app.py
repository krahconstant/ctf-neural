...
# ── Imports optionnels (graceful degradation si absent) ──────────────────────
_OPTIONAL_LIBS = {}
for _lib, _imp in [
    ("gmpy2",    "gmpy2"),
    ("pwn",      "pwn"),
    ...
    try:
        _OPTIONAL_LIBS[_lib] = __import__(_imp)
    except ImportError:
        _OPTIONAL_LIBS[_lib] = None

def has_lib(name):
    return _OPTIONAL_LIBS.get(name) is not None

AVAILABLE_LIBS = [k for k, v in _OPTIONAL_LIBS.items() if v is not None]
...
# Dépendances système nécessaires pour certains paquets
_SYS_DEPS = {
    "gmpy2":    "libgmp-dev libmpfr-dev libmpc-dev",
    "pwntools": "libssl-dev libffi-dev",
    "angr":     "libffi-dev",
}

def pip_install(packages):
    """Installe des paquets pip avec fallback et dépendances système."""
    ...
    for pkg in safe:
        ...
        if base in _SYS_DEPS:
            sys_pkgs.update(_SYS_DEPS[base].split())
    if sys_pkgs:
        run(f"apt-get install -y -qq {' '.join(sys_pkgs)} 2>/dev/null || true", timeout=60)

    # Essai pip standard puis avec --break-system-packages
    r = run(f"pip install --quiet {' '.join(safe)}", timeout=120)
    if r["code"] != 0:
        run(f"pip install --quiet --break-system-packages {' '.join(safe)}", timeout=120)
...
Sois TRÈS précis. Cite les valeurs exactes du code.""",
        ...
        # Priorité à l'adresse fournie par l'utilisateur > celle du LLM
        remote_hint = user_remote or recon.get("remote_hint") or ""
        # Propager dans recon pour la gestion remote
        if user_remote:
            recon["needs_remote"] = True
            recon["remote_hint"]  = user_remote
        script = ""
        try:
...
7. IMPORTANT — Libs garanties dispo : sympy, numpy, PIL (Pillow), requests, scapy, z3
   Libs PEUT-ÊTRE dispo (vérifie avant) : pycryptodome (Crypto), pwntools (pwn), gmpy2, capstone, pefile
   Si pycryptodome absent → hashlib, struct, binascii (stdlib)
   Si pwntools absent → import socket; s=socket.create_connection((HOST,PORT)); s.sendall(b"data")
   Si gmpy2 absent → pow(base, exp, mod) natif Python est suffisant pour RSA
   Si z3 absent → sympy ou brute-force borné
   Toujours mettre try/except ImportError avec fallback stdlib
...
- ImportError/ModuleNotFoundError :
  * pwntools absent → import socket; s=socket.create_connection((HOST,PORT))
  * gmpy2 absent → pow(a,b,n) natif Python suffit pour RSA, ou sympy.ntheory
  * pycryptodome absent → from hashlib import md5,sha256; import struct, binascii
  * capstone absent → subprocess.run(["objdump","-d","binary"])
  * scapy absent → from struct import pack,unpack pour parser les paquets
- FileNotFoundError → fichiers dans le dossier courant, utilise os.listdir('.')
- TypeError/ValueError → encode/decode explicite, long_to_bytes via struct ou int.to_bytes
- Pas de flag dans output → ajoute print("FLAG:", flag) explicite + print(type(flag))
- Timeout → borne les boucles, utilise des algos plus rapides (pow mod natif)
- SyntaxError → vérifie indentation, quotes, parenthèses
...
def build_ctx(info, desc, hint="", user_remote=""):
    ...
    if desc: ctx += f"\n--- DESCRIPTION DU DÉFI ---\n{desc}\n"
    if user_remote: ctx += f"\n--- ADRESSE SERVICE DISTANT (fournie par l'utilisateur) ---\n{user_remote}\n"
    # Informe le LLM des libs réellement disponibles
    ctx += f"\n--- LIBS PYTHON DISPONIBLES SUR CE SERVEUR ---\n{', '.join(AVAILABLE_LIBS)}\n"
